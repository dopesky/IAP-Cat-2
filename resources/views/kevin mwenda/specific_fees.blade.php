...
<div id="container">
	<h1><b>Here Are Specific Fee Payments for <?=ucwords(strtolower($fees['last_name']." ".$fees['first_name']))?>.</b><span class="link"><a href="{{url('')}}" class="text-info">Landing Page</a></span></h1>
	...
	<div id="body">
		<table class="table bordered table-striped table-hover w-100">
			...
			<tbody>
				...
				<tr>
					<td><?=ucwords(strtolower($fees['last_name']." ".$fees['first_name']))?></td>
					<td><?=date_create($fees['dob'])->format('dS M, Y')?></td>
					<td><?=$fees['address']?></td>
					<td><?=$fee['amount']?></td>
